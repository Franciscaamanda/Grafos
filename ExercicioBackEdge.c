...
int main(int *argc, char *argv[])
{
    ...
    scanf("%d",&qtd_vertices); //quantidade de vértices
    ...
    while(i<qtd_vertices) //só serve se for pra árvores
    {
...
void dfs(vertice * vertices , int x, int distancia)
{
    ...
    while(aux!=NULL)
    {
        ...
        aux = aux->prox;
        if (aux==NULL) //se acabar e ter que sair da lista, soma 1 e adiciona no vetor de saída
        {
            cont+=1;
            vertices[x].out = cont;
